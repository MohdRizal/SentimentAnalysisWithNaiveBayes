...
ECS (Enhanched Confix Stripping) Algorithm 
*/
...
function cekKamus($kata){
	//global $basicWords;
	global $con;
	$sql = mysqli_query($con, "SELECT * FROM katadasar_ecs WHERE nama = '$kata'");
	$basicWord = mysqli_fetch_array($sql, MYSQLI_ASSOC);
	//$dbhelper->from("katadasar")->where("nama = '$kata'")->get("*", true);
	//foreach($getBasicWorlds AS $gbw){
	//	$basicWords[$gbw->nama] = $gbw->nama;
	//}
	
	if (isset($basicWord['nama'])) {
		return $basicWord['nama']; // True jika ada
	}else{
